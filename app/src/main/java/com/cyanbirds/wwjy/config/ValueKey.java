package com.cyanbirds.wwjy.config;

/**
 * 
 * @ClassName:ValueKey
 * @Description:传递值可以的声明
 * @author Administrator
 * @Date:2015年5月16日下午3:34:50
 *
 */
public class ValueKey {

	/** webview url */
	public final static String WEB_URL_KEY = "url_key";
	/** 电话号码 */
	public final static String PHONE_NUMBER = "phone_number";
	/** Cookie */
	public final static String COOKIE = "cookie";
	/** 区号 */
	public final static String AREA_CODE = "area_code";
	/** 短信验证码 */
	public final static String SMS_CODE = "sms_code";
	/** 聊天对象 */
	public final static String TALKER = "talker";
	/** 经度 */
	public final static String LATITUDE = "latitude";
	/** 纬度 */
	public final static String LONGITUDE = "longitude";
	/** 会话id */
	public final static String CONVERSATION_ID = "conversation_id";
	/** 搜索关键词 */
	public final static String SEARCH_KEYWORD = "search_keyword";
	/** 图片url */
	public final static String IMAGE_URL = "image_url";
	/** 图片数量 */
	public final static String IMAGE_COUNT = "image_count";
	/** 消息id */
	public final static String MSG_ID = "msg_id";
	/** 选择的图片地址 */
	public final static String PHOTO_PATHS = "photo_paths";
	/**位置**/
	public final static String POSITION = "position";
	/**地址**/
	public final static String ADDRESS = "address";

	/**账户**/
	public final static String USER = "user";
	/**账户**/
	public final static String ACCOUNT = "account";
	/**账号**/
	public static final String USER_ID = "user_id";
	/**密码**/
	public final static String PASSWORD = "password";
	/** 用户名 */
	public static final String USER_NAME = "user_name";
	/** 联系电话 */
	public static final String MOBILE = "mobile";
	/** 性别 */
	public static final String SEX = "sex";
	/** 生日 */
	public static final String BIRTHDAY = "birthday";
	/** 个性签名 */
	public static final String SIGNATURE = "signature";
	/** 头像url */
	public static final String FACE_URL = "face_url";
	/** 本地头像地址 */
	public static final String FACE_LOCAL = "face_local";
	/** 微信 */
	public static final String WEIXIN_NO = "weixin_no";
	/** qq */
	public static final String QQ_NO = "qq_no";
	/** 年龄 */
	public static final String AGE = "age";
	/** 身高 */
	public static final String TALL = "tall";
	/** 情感状态 */
	public static final String STATE_MARRY = "state_marry";
	/** 星座 */
	public static final String CONSTELLATION = "constellation";
	/** 会话 */
	public static final String CONVERSATION = "conversation";
	/** 输入电话号码的目的   0：登录或者注册  1：找回密码 */
	public static final String INPUT_PHONE_TYPE = "input_phone_type";
	/**来源**/
	public static final String FROM_ACTIVITY = "from_activity";
	/**视频列表**/
	public static final String VIDEO_LIST = "video_list";
	/**视频**/
	public static final String VIDEO = "video";
	/** 动态  **/
	public static final String DYNAMIC = "dynamic";
	/** apkUrl  **/
	public static final String APK_URL = "apk_url";
	/** data  **/
	public static final String DATA = "data";
	/** user_type  **/
	public static final String USER_TYPE = "user_type";
	/** LOCATION  **/
	public static final String LOCATION = "location";
	/**视频类型**/
	public static final String VIDEO_TYPE = "video_type";
}
